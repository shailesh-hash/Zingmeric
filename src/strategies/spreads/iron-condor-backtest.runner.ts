...
import type { IronCondorStrategy } from './iron-condor.strategy.js';
import { calculateIronCondorCloseCost } from './option-chain.utils.js';
import type { IronCondorPosition } from './iron-condor.types.js';
import { SignalAction } from '../types/signal.type.js';

export interface IronCondorBacktestCandle {
  timestamp: Date;
  close: number;
  optionChain: OptionChainSnapshot;
}

export interface IronCondorBacktestConfig {
  instrumentId: string;
  symbol: string;
...
export interface IronCondorBacktestResult {
  instrumentId: string;
  symbol: string;
  strategyName: string;
  startDate: Date;
...
export function runIronCondorBacktest(
  strategy: IronCondorStrategy,
  config: IronCondorBacktestConfig,
  candles: IronCondorBacktestCandle[],
  ...
  let openCredit = 0;
  let openPosition: IronCondorPosition | null = null;
  ...
  for (const replayCandle of sortedCandles) {
    ...
    if (signal.action === SignalAction.BUY && strategy.openPosition) {
      ...
      openPosition = strategy.openPosition;
      cash += openCredit;
    ...
    if (signal.action === SignalAction.SELL && openPosition) {
      const closeCost =
        calculateIronCondorCloseCost(
          openPosition.putSpread,
          openPosition.callSpread,
          sourceCandle.optionChain.puts,
          sourceCandle.optionChain.calls,
        ) ?? 0;
